...
public class ExpenseService {
  ...
  public void saveNewExpense(ExpenseDTO dto, Long userId) {
    Category category = categoryRepository.findById(dto.category_id()).orElseThrow(() -> new RuntimeException("Categoria não encontrada"));
  ...
  public void updateLastExpense(Long userId, ExpenseDTO dto) {
    Expense lastExpense = expenseRepository.findTopByUserIdOrderByCreatedAtDesc(userId).orElseThrow(() -> new RuntimeException("Nenhuma despesa encontrada para o usuário"));
    Category category = categoryRepository.findById(dto.category_id()).orElseThrow(() -> new RuntimeException("Categoria não encontrada"));
  ...
  public ExpensesSummaryInPeriodDTO summaryExpensesInPeriod(Long userId, LocalDate from, LocalDate to) {
    ...
    return new ExpensesSummaryInPeriodDTO(top.categoryName(), top.totalCategoryExpense(), total);
  }

  public List<CategoryPercentageDTO> getCategoryPercentages(Long userId, LocalDate from, LocalDate to) {
    var expensesInPeriod = findExpensesInPeriod(userId, from, to);
    BigDecimal total = calculateTotal(expensesInPeriod);
    if (total.compareTo(BigDecimal.ZERO) == 0) {
      return Collections.emptyList();
    }
    var sumByCat = expensesInPeriod.stream().collect(Collectors.groupingBy(expense -> expense.getCategory().getName(), Collectors.reducing(BigDecimal.ZERO, Expense::getAmount, BigDecimal::add)));
    return sumByCat.entrySet().stream().map(entry -> {
      BigDecimal categoryPercentage = entry.getValue().multiply(BigDecimal.valueOf(100)).divide(total, 2, RoundingMode.HALF_UP);
  ...
  public List<Expense> findExpensesInPeriod(Long userId, LocalDate from, LocalDate to) {
    ...
    return expenseRepository.findByUserIdAndCreatedAtBetweenOrderByCreatedAtDesc(userId, dtFrom, dtTo);
  }
  ...
  private CategorySummary frequencyCategory(List<Expense> expenses) {
    Map<String, BigDecimal> sumByCategory = expenses.stream().collect(Collectors.groupingBy(e -> e.getCategory().getName(), Collectors.reducing(BigDecimal.ZERO, Expense::getAmount, BigDecimal::add)));

    if (sumByCategory.isEmpty()) {
      return new CategorySummary("Nenhuma categoria no período", BigDecimal.ZERO);
    }

    var topEntry = Collections.max(sumByCategory.entrySet(), Map.Entry.comparingByValue());
